...
#import "BBMediaViewController.h"
...
#import "BBCameraFocusView.h"
#import "BBCameraRecordProgressView.h"
...
@interface BBCameraViewController ()<UIGestureRecognizerDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate,PBJVisionDelegate>
/**
 *  顶部工具条
 */
 ...
 *  闪光灯控制
 */
@property (nonatomic ,strong) BBTintedButton               *flashButton;
/**
 *  拍摄按钮
 */
 ...
 *  相册按钮
 */
@property (nonatomic ,strong) BBTintedButton               *albumButton;
/**
 *  相机类型切换按钮
 */
 ...
 *  遮罩上半部分
 */
@property (nonatomic ,strong) BBCameraSlideUpView          *slideUpView;
/**
 *  遮罩下半部分
 */
@property (nonatomic ,strong) BBCameraSlideDownView        *slideDownView;
/**
 *  网格辅助线
 */
 ...
 *  对焦辅助View
 */
@property (nonatomic ,strong) BBCameraFocusView            *focusView;
/**
 *  录像进度条
 */
@property (nonatomic ,strong) BBCameraRecordProgressView   *recordProgressView;
/**
 *  对焦手势
 */
...
- (void)viewDidLoad
{
    ...
    if ([[BBCamera getOption:kBBCameraOptionHiddenToggleButton] boolValue] == YES) {
        _toggleButton.hidden = YES;
        _toggleButtonWidth.constant = 0;
    }
    
    if ([[BBCamera getOption:kBBCameraOptionHiddenAlbumButton] boolValue] == YES) {
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [BBCameraSlideView hideSlideUpView:self.slideUpView
...
{
    UIImage *photo = [BBAlbum imageWithMediaInfo:info];
    
    BBPhotoViewController *viewController = [BBPhotoViewController newWithDelegate:_delegate photo:photo];
...
- (void)albumTapped {
    ...
    [self showSlideViewWithCompletion:^{
        UIImagePickerController *pickerController = [BBAlbum imagePickerControllerWithDelegate:self];
...
- (void)handleFocusTapGesterRecognizer:(UIGestureRecognizer *)gestureRecognizer {
    ...
    CGPoint adjustPoint = [PBJVisionUtilities convertToPointOfInteresBBromViewCoordinates:tapPoint
...
- (BBCameraSlideUpView *)slideUpView {
    if (!_slideUpView) {
        _slideUpView = [[BBCameraSlideUpView alloc] init];
...
- (BBCameraSlideDownView *)slideDownView {
    if (!_slideDownView) {
        _slideDownView = [[BBCameraSlideDownView alloc] init];
...
- (BBCameraGridView *)gridView {
    if (_gridView == nil) {
        _gridView = [[BBCameraGridView alloc] initWithFrame:_previewLayer.bounds];
...
- (BBCameraFocusView *)focusView {
    if (!_focusView) {
        _focusView = [[BBCameraFocusView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
    }
    return _focusView;
}

- (BBCameraRecordProgressView *)recordProgressView {
    if (!_recordProgressView) {
        _recordProgressView = [[BBCameraRecordProgressView alloc] initWithFrame:CGRectZero];
...
- (UIButton *)closeButton {
    if (!_closeButton) {
        _closeButton = [UIButton BBcCreateButtonWithImage:@"CameraClose.png"
...
- (UIButton *)nextButton {
    if (!_nextButton) {
        _nextButton = [UIButton BBcCreateButtonWithTitle:@"下一步"
                                                  target:self
                                                selector:@selector(nextTapped)];
        _nextButton.tintColor = [BBCameraColor tintColor];
...
- (UIButton *)gridButton {
    if (!_gridButton) {
        _gridButton = [UIButton BBcCreateButtonWithImage:@"CameraGrid.png"
...
- (UIButton *)toggleButton {
    if (!_toggleButton) {
        _toggleButton = [UIButton BBcCreateButtonWithImage:@"CameraToggle.png"
...
- (BBTintedButton *)flashButton {
    if (!_flashButton) {
        _flashButton = [BBTintedButton buttonWithType:UIButtonTypeCustom];
...
- (UIButton *)shotButton {
    if (!_shotButton) {
        _shotButton = [UIButton BBcCreateButtonWithImage:@"CameraShot.png"
...
- (BBTintedButton *)albumButton {
    if (!_albumButton) {
        _albumButton = [BBTintedButton buttonWithType:UIButtonTypeCustom];
...
- (UIButton *)videoButton {
    if (!_videoButton) {
        _videoButton = [UIButton BBcCreateButtonWithImage:@"CameraHandleVideo.png"
...
- (UIButton *)deleteVideoButton {
    if (!_deleteVideoButton) {
        _deleteVideoButton = [UIButton BBcCreateButtonWithImage:@"CameraHandleVideo.png"
                                                   imagePressed:@"CameraHandleVideo.png"
                                                         target:self
    ...
    [BBCameraSlideView hideSlideUpView:self.slideUpView
                            ...
                            completion:^
     {
         ...
         if ([[PBJVision sharedInstance] cameraMode] == PBJCameraModePhoto) {
             ...
             _recordProgressView.hidden = YES;
             _nextButton.hidden = YES;
             [_videoButton BBcUpdateButtonImage:@"CameraHandleVideo.png"
                                   imagePressed:@"CameraHandleVideo.png"];
         }
         else {
             //视频模式
             ...
             [_videoButton BBcUpdateButtonImage:@"CameraHandlePhoto.png"
...
- (void)showSlideViewWithCompletion:(void (^)(void))completion {
    ...
    [BBCameraSlideView showSlideUpView:self.slideUpView
                         slideDownView:self.slideDownView
                                atView:_captureView
                            completion:^{
                                completion();
...
- (void)vision:(PBJVision *)vision capturedPhoto:(nullable NSDictionary *)photoDict error:(nullable NSError *)error {
    ...
    BBMediaFileModel *mediaFile = [[BBMediaFileModel alloc] init];
    mediaFile.fileType = BBMediaFileTypePhoto;
    mediaFile.image = [photoDict objectForKey:PBJVisionPhotoImageKey];
    BBMediaViewController *viewController = [BBMediaViewController newWithDelegate:_delegate
                                                                         mediaFile:mediaFile];
...
- (void)visionWillStarBBocus:(PBJVision *)vision
{
}
