...
@interface BBImageCropViewController() {
...
@property (nonatomic,weak   ) IBOutlet UIView<BBImageEditorFrame> *frameView;
...
@implementation BBImageCropViewController
...
- (void)viewDidLoad
{
    ...
    if (!_frameView) {
        BBImageEditorFrameView *view = [[BBImageEditorFrameView alloc] initWithFrame:self.view.bounds];
...
- (UIToolbar *)toolBar {
    if (!_toolBar) {
        _toolBar = [[UIToolbar alloc] initWithFrame:
                    CGRectMake(0, self.view.frame.size.height - kDefaultToolBarHeight,
                               self.view.frame.size.width, kDefaultToolBarHeight)];
...
- (void)checkBoundsWithTransform:(CGAffineTransform)transform
{
    ...
    if(CGRectContainsRect([self CGRecBBromRectangle:r3],r1)) {
        self.validTransform = transform;
    }
}
...
-(CGRect)CGRecBBromRectangle:(Rectangle)rect
{
    return (CGRect) {
        .origin = rect.tl,
