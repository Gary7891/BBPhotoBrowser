...
#import "BBTapDetectingImageView.h"
...
static const CGFloat BBMotionViewRotationMinimumTreshold = 0.1f;
static const CGFloat BBMotionGyroUpdateInterval = 1 / 100;
static const CGFloat BBMotionViewRotationFactor = 4.0f;

@interface BBZoomingScrollView()<UIScrollViewDelegate,BBTapDetectingViewDelegate,BBTapDetectingImageViewDelegate> {
    BBPhotoBrowser __weak   *_photoBrowser;
    BBTapDetectingView      *_tapView; // for background taps
...
@implementation BBZoomingScrollView
...
- (id)initWithPhotoBrowser:(BBPhotoBrowser *)browser {
    if ((self = [super init])) {
        ...
        _tapView = [[BBTapDetectingView alloc] initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[BBTapDetectingImageView alloc] initWithFrame:CGRectZero];
...
//        _photoImageView.layer.borderWidth = 5;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(setProgressFromNotification:)
                                                     name:BBPHOTO_PROGRESS_NOTIFICATION
                                                   object:nil];
...
- (void)setPhoto:(id<BBPhoto>)photo {
...
//        NSLog(@"view = %@",view);
        if ([view isKindOfClass:[BBPhotoTagView class]]) {
            view.hidden = YES;
...
- (void)setProgressFromNotification:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        id <BBPhoto> photoWithProgress = [dict objectForKey:@"photo"];
...
- (void)startMonitoring {
    ...
    _motionRate = _photoImageView.frame.size.width / self.frame.size.width * BBMotionViewRotationFactor;
    ...
    if (!_motionManager) {
        ...
        _motionManager.gyroUpdateInterval = BBMotionGyroUpdateInterval;
    }
    ...
    if (![_motionManager isGyroActive] && [_motionManager isGyroAvailable] ) {
        [_motionManager startGyroUpdatesToQueue:[NSOperationQueue currentQueue]
                                    withHandler:^(CMGyroData *gyroData, NSError *error)
         {
             ...
             if (fabs(rotationRate) >= BBMotionViewRotationMinimumTreshold) {
...
- (void)startNewTagPopover:(BBPhotoTagView *)popover
         atNormalizedPoint:(CGPoint)normalizedPoint
              pointOnImage:(CGPoint)pointOnImage
                      size:(CGSize)sizeOnImage
{
    BBPLog(@"normalized.x=%@,y=%@",@(normalizedPoint.x),@(normalizedPoint.y));
...
//        [popover becomeFirstResponder];
        popover.text = BBPhotoBrowserLocalizedStrings(@"This is?");
    }
}
