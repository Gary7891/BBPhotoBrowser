//
//  BBCollectionPickerController.m
//  BBPhotoBrowser
//
...
#import "BBCollectionCell.h"
#import "PHCollection+BBThumbnail.h"


@interface BBCollectionPickerController () <PHPhotoLibraryChangeObserver, UIViewControllerRestoration>
{
...
@end
@implementation BBCollectionPickerController
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[BBCollectionCell class] forCellReuseIdentifier:@"CollectionCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < [self _momentsSections]) {
        BBCollectionCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CollectionCell" forIndexPath:indexPath];
        
        cell.titleLabel.text = BBPhotoBrowserLocalizedStrings(@"Moments");
        cell.thumbnailView.image = BBPhotoBrowserImageNamed(@"BBLibraryCollectionIcon");
        [PHCollection bb_requestThumbnailForMomentsWithAssetsFetchOptions:self.assetFetchOptions
        ...
        if ([collection isKindOfClass:[PHAssetCollection class]]) {
            ...
            cell.thumbnailView.image = BBPhotoBrowserImageNamed(@"BBLibraryCollectionIcon");
            [collection bb_requestThumbnailWithAssetsFetchOptions:self.assetFetchOptions completion:^(UIImage *result) {
                if (result != nil) {
                    cell.thumbnailView.image = result;
                }
            }];
            ...
            cell.thumbnailView.image = BBPhotoBrowserImageNamed(@"BBLibraryCollectionList");
            [collection bb_requestThumbnailWithAssetsFetchOptions:self.assetFetchOptions
                                                       completion:^(UIImage *result)
             {
                 if (result != nil) {
                     cell.thumbnailView.image = result;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < [self _momentsSections]) {
        ...
        if ([collection isKindOfClass:[PHAssetCollection class]]) {
            ...
            BBCollectionPickerController *picker = [[BBCollectionPickerController alloc] init];
...
- (void)decodeRestorableStateWithCoder:(NSCoder *)coder
{
    ...
    id<BBCollectionPickerControllerDelegate> delegate = [coder decodeObjectForKey:@"delegate"];
    if (delegate != nil) {
