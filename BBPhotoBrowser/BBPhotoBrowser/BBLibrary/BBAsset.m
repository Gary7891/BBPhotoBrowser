...
@property (nonatomic, assign) BBAssetType    type;
...
- (void)initProperty {
    ...
    self.type = BBAssetTypeUnInitiliazed;
...
- (double)duration {
    if (_duration == 0 && _type == BBAssetTypeVideo) {
        if (self.isPHAsset) {
...
- (BBAssetType)type {
    if (_type == BBAssetTypeUnInitiliazed) {
        if (self.isPHAsset) {
            switch (self.phAsset.mediaType) {
                case PHAssetMediaTypeUnknown: {
                    self.type = BBAssetTypeUnknown;
                    break;
                }
                case PHAssetMediaTypeImage: {
                    self.type = BBAssetTypePhoto;
                    break;
                }
                case PHAssetMediaTypeVideo: {
                    self.type = BBAssetTypeVideo;
                    break;
                }
                case PHAssetMediaTypeAudio: {
                    self.type = BBAssetTypeAudio;
                    break;
                }
                default: {
                    self.type = BBAssetTypeUnknown;
...
- (BOOL)isVideo {
    return self.type == BBAssetTypeVideo;
}

- (BOOL)isPhoto {
    return self.type == BBAssetTypePhoto;
}
    ...
    if (range.length > 0) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            [libary assetForURL:[NSURL URLWithString:localIdentifier] resultBlock:^(ALAsset *asset){
                bbAsset = [[BBAsset alloc]initWithALAsset:asset];
                
                NSLog(@"%@", bbAsset.localIdentifier);
