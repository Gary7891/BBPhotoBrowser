...
#import "BBLibraryCollectionViewCell.h"
...
static NSString * const kBBLCollectionIdentifier        = @"kBBLCollectionIdentifier";
static NSString * const kBBLCollectionCameraIdentifier  = @"kBBLCollectionCameraIdentifier";
static NSString * const kBBLCollectionLibraryIdentifier = @"kBBLCollectionLibraryIdentifier";

@interface BBLibraryViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,PHPhotoLibraryChangeObserver,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, strong) BBImageCropViewController *editorViewController;
...
@implementation BBLibraryViewController
...
- (void)performLoadAssets {
    // Load
    self.navigationItem.title = BBPhotoBrowserLocalizedStrings(@"Loading Photos");
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (PHAsset *asset in weakSelf.assetsFetchResults) {
            if (asset.mediaType == PHAssetMediaTypeImage) {
                BBAsset *bbAsset = [BBAsset assetFromPH:asset];
                [_items addObject:bbAsset];
            }
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            weakSelf.navigationItem.title = BBPhotoBrowserLocalizedStrings(@"All Photos");
...
-(UICollectionView *) collectionView {
    if (!_collectionView) {
        BBLibraryViewLayout *flowLayout = [[BBLibraryViewLayout alloc] init];
        ...
        [_collectionView registerClass:[BBLibraryCollectionViewCell class]
            forCellWithReuseIdentifier:kBBLCollectionLibraryIdentifier];
        [_collectionView registerClass:[BBLibraryCameraCollectionViewCell class]
            forCellWithReuseIdentifier:kBBLCollectionCameraIdentifier];
...
//                     forState:UIControlStateNormal];
        ...
        [_selectedButton setTitle:isM ? [NSString stringWithFormat:BBPhotoBrowserLocalizedStrings(@"Photos Selected Done"),[self.selectedAssets count],_maximumNumberOfSelection]:BBPhotoBrowserLocalizedStrings(@"Done")
                         forState:UIControlStateNormal];
}
...
- (void)selectItem:(NSIndexPath *)indexPath asset:(BBAsset *)asset {
    if (!_selectedAssets) {
        _selectedAssets = [NSMutableArray array];
    }
    if (!_removeAssets) {
        ...
        if (![collectionChanges hasIncrementalChanges] || [collectionChanges hasMoves]) {
            ...
            [collectionView performBatchUpdates:^{
                ...
                if ([removedIndexes count] > 0) {
                    [collectionView deleteItemsAtIndexPaths:[removedIndexes BBl_indexPathsFromIndexesWithSection:0]];
                }
                
                NSIndexSet *insertedIndexes = [collectionChanges insertedIndexes];
                if ([insertedIndexes count] > 0) {
                    [collectionView insertItemsAtIndexPaths:[insertedIndexes BBl_indexPathsFromIndexesWithSection:0]];
                }
                
                NSIndexSet *changedIndexes = [collectionChanges changedIndexes];
                if ([changedIndexes count] > 0) {
                    [collectionView reloadItemsAtIndexPaths:[changedIndexes BBl_indexPathsFromIndexesWithSection:0]];
                }
            } completion:NULL];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.item == 0 ) {
        BBLibraryCameraCollectionViewCell *cameraCell = [collectionView dequeueReusableCellWithReuseIdentifier:kBBLCollectionCameraIdentifier forIndexPath:indexPath];
        [cameraCell startCamera];
        return cameraCell;
    }
    BBLibraryCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kBBLCollectionLibraryIdentifier forIndexPath:indexPath];
    BBAsset *asset = self.items[indexPath.item];
    if (asset.isPHAsset) {
    ...
    if (indexPath.section == 0 && indexPath.item == 0) {
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])  {
            ...
            [self presentViewController:imagePickerController
                               animated:YES
                             completion:^{
                                 BBLibraryCameraCollectionViewCell *item = [collectionView dequeueReusableCellWithReuseIdentifier:kBBLCollectionCameraIdentifier
        ...
        BBAsset *asset = [self.items objectAtIndex:indexPath.item];
        if (asset.size.width > 0) {
            if (self.allowsMultipleSelection) {
                //多选
                if (asset.isImageResultIsInCloud) {
                    [SVProgressHUD showInfoWithStatus:@"图片需要从iCloud进行下载"];
                    __block BBLibraryCollectionViewCell *cell = (BBLibraryCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
                    //图片需要从iCloud下载
                    [collectionView deselectItemAtIndexPath:indexPath animated:NO];
                }
                else {
...
- (void)cropImageByALAsset:(BBAsset *)asset {
    //使用图片剪裁功能
    ...
    if (!_editorViewController) {
        _editorViewController = [[BBImageCropViewController alloc] init];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(self.collectionView.bounds) / 3.0f) {
        ...
        [self computeDifferenceBetweenRect:self.previousPreheatRect
         ...
         {
             NSArray *indexPaths = [self.collectionView BBl_indexPathsForElementsInRect:removedRect];
             [removedIndexPaths addObjectsFromArray:indexPaths];
         } addedHandler:^(CGRect addedRect) {
             NSArray *indexPaths = [self.collectionView BBl_indexPathsForElementsInRect:addedRect];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if(success){
            ...
            BBAsset *asset = [BBAsset assetFromLocalIdentifier:placeholderAsset.localIdentifier];
