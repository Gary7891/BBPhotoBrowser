...
#import "BBiCloudDownloadHelper.h"

#define BBObjectSpacing 2.0
...
- (void)selectAsset:(PHAsset *)asset {
    if ([_selectedAssets count] >= _maxSelectedCount) {
        [SVProgressHUD showInfoWithStatus:[NSString localizedStringWithFormat:BBPhotoBrowserLocalizedStrings(@"Choose %d photos at most"), _selectedAssets.count]];
        return;
    }
...
- (void)_updateForAssetCollection
{
    ...
    if (status != PHAuthorizationStatusAuthorized) {
        ...
        label.text = BBPhotoBrowserLocalizedStrings(@"Open album privacy settings");
        ...
        [button setTitle:BBPhotoBrowserLocalizedStrings(@"Open settings") forState:UIControlStateNormal];
        ...
        self.navigationItem.title = BBPhotoBrowserLocalizedStrings(@"Request album permissions");
        return;

    }
    if (_assetCollection == nil) {
        self.title = BBPhotoBrowserLocalizedStrings(@"Moments");
    } else {
...
- (void)saveHeaderModels {
    ...
    for (PHAssetCollection *collection in _moments) {
        ...
        if (currentDateComponents.year>startDateComponents.year) {
            [formatter setDateFormat:BBPhotoBrowserLocalizedStrings(@"dateFormatterOverYear")];
            
            dateStr = [formatter stringFromDate:collection.startDate];
        }
        else if (components.day>6) {
            
            [formatter setDateFormat:BBPhotoBrowserLocalizedStrings(@"dateFormatterOverWeek")];
            dateStr = [formatter stringFromDate:collection.startDate];
            
        }
        else if (components.day>1) {
            [formatter setDateFormat:BBPhotoBrowserLocalizedStrings(@"dateFormatterOverInWeek")];
            dateStr = [formatter stringFromDate:collection.startDate];
        }
        else if (components.day==1) {
            dateStr = BBPhotoBrowserLocalizedStrings(@"Yesterday");
        }
        else if (components.day==0) {
            dateStr = BBPhotoBrowserLocalizedStrings(@"Today");
        }
        if (collection.localizedTitle) {
            model.reuseIdentifier = BBMomentHeaderViewDetailIdentifier;
            model.primary = collection.localizedTitle;
            model.secondary = [collection.localizedLocationNames componentsJoinedByString:@"、"];
            [formatter setDateFormat:BBPhotoBrowserLocalizedStrings(@"dateFormatterOverWeekDetail")];
            model.detail = [formatter stringFromDate:collection.startDate];
        }
        else {
            model.reuseIdentifier = BBMomentHeaderViewNomalIdentifier;
...
- (void)_updateDoneButton {
    ...
    if ([self.delegate respondsToSelector:@selector(imagePickerControllerTitleForDoneButton:)]) {
        ...
        title = [NSString stringWithFormat:BBPhotoBrowserLocalizedStrings(@"Photos Selected Done"),[self.selectedAssets count],_maxSelectedCount];
    } else {
        title = BBPhotoBrowserLocalizedStrings(@"Done");
    }
...
- (void)_updateSelectAllButton {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (allSelected) {
                _selectAllButton.title = BBPhotoBrowserLocalizedStrings(@"Deselect All");
                _selectAllButton.action = @selector(deselectAll:);
            } else {
                _selectAllButton.title = BBPhotoBrowserLocalizedStrings(@"Select All");
...
//    _cameraButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCamera target:self action:@selector(takePicture:)];
    ...
    _pasteButton = [[UIBarButtonItem alloc] initWithTitle:BBPhotoBrowserLocalizedStrings(@"Paste") style:UIBarButtonItemStylePlain target:self action:@selector(paste:)];
...
- (instancetype)init {
    ...
    layout.minimumInteritemSpacing = BBObjectSpacing;
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[BBMomentHeaderNomalView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:BBMomentHeaderViewNomalIdentifier];
    [self.collectionView registerClass:[BBMomentHeaderDetailView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:BBMomentHeaderViewDetailIdentifier];
...
- (IBAction)changeCollection:(id)sender {
    ...
    if (_selectedAssets.count > 0) {
        PHAssetCollection *collection = [PHAssetCollection transientAssetCollectionWithAssets:_selectedAssets.array title:BBPhotoBrowserLocalizedStrings(@"Selected")];
...
- (void)updateHeaderView:(NSIndexPath*)indexPath {
    ...
    BBMomentHeaderNomalView *headerView = [headerViewDictionary objectForKey:[NSString stringWithFormat:@"%@",@(indexPath.section)]];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat width =  (collectionView.bounds.size.width-BBObjectSpacing*3)/4.0;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    BBMomentHeaderModel *model = _headerModelsArr [indexPath.section];
    BBMomentHeaderDetailView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:model.reuseIdentifier forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    }
    BBAssetCell *cell = (BBAssetCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void)assetCellView:(BBAssetCell *)cell didDownloadAtIndexPath:(NSIndexPath *)indexPath {
    BBiCloudDownloadHelper *helper = [BBiCloudDownloadHelper sharedHelper];
    ...
    [helper startDownLoadWithAsset:cell.asset
                   progressHandler:^(double progress, NSError * _Nullable error, BOOL * _Nonnull stop, NSDictionary * _Nullable info) {
                       dispatch_async(dispatch_get_main_queue(), ^{
                           ...
                           [[NSNotificationCenter defaultCenter] postNotificationName:BBImagePickeriCloudDownLoading object:progressNumber];
                       });
                   } finined:^{
                       __strong typeof(weakself) strongself = weakself;
                       dispatch_async(dispatch_get_main_queue(), ^{
                           [[NSNotificationCenter defaultCenter] postNotificationName:BBImagePickeriCloudDownLoadFinish object:nil];
