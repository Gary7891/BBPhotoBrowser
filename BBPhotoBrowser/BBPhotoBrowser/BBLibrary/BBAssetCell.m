...
- (UIButton *)selectedBadgeButton {
    if (!_selectedBadgeButton) {
        ...
        [_selectedBadgeButton setImage:[UIImage imageNamed:@"BBLibraryResource.bundle/images/BBLibraryCollectioniCloud.png"] forState:UIControlStateDisabled];
...
- (void)_init {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didDownloadQualityImage) name:BBImagePickeriCloudDownLoadFinish object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downloadingQualityImage:) name:BBImagePickeriCloudDownLoading object:nil];
...
- (void)downloadingQualityImage:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([[BBiCloudDownloadHelper sharedHelper].asset isEqual:self.asset]) {
            self.selectedBadgeButton.hidden = YES;
            self.downloadIndicator.hidden = NO;
