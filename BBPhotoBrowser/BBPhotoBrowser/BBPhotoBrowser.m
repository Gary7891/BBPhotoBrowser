...
#import "BBPhotoBrowserPrivate.h"
...
@interface BBPhotoBrowser() <BBPhotoTagViewDelegate> {
    BOOL            _tagOnView;
    CGPoint         _editPoint;
    BBPhotoTagView  *_currentTagView;
    BOOL            _willlayout;
}

@end

static void * BBVideoPlayerObservation = &BBVideoPlayerObservation;

@implementation BBPhotoBrowser
...
- (void)_initialisation {
    ...
    // Listen for BBPhoto notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBBPhotoLoadingDidEndNotification:)
                                                 name:BBPHOTO_LOADING_DID_END_NOTIFICATION
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
//                                                                action:@selector(doneButtonPressed:)];
    ...
    _doneButton = [[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:BBPhotoBrowserLocalizedStrings(@"Done Process"),@(current),@(total)] style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
    
    _backButton = [[UIBarButtonItem alloc]initWithTitle:BBPhotoBrowserLocalizedStrings(@"Back") style:UIBarButtonItemStyleDone target:self action:@selector(backButtonPressed:)];
...
- (void)performLayout {
    ...
    // Content offset
    _pagingScrollView.contentOffset = [self contentOffseBBorPageAtIndex:_currentPageIndex];
    [self tilePages];
...
- (void)performCloseAnimationWithScrollView:(BBZoomingScrollView *)scrollView {
...
- (void)panGestureRecognized:(id)sender {
    // Initial Setup
    BBZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_viewHasAppearedInitially) {
        if (_autoPlayOnAppear) {
            BBPhoto *photo = [self photoAtIndex:_currentPageIndex];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if (parent && _hasBelongedToViewController) {
        [NSException raise:@"BBPhotoBrowser Instance Reuse" format:@"BBPhotoBrowser instances cannot be reused."];
    }
}
...
- (void)layoutVisiblePages {
    ...
    for (BBZoomingScrollView *page in _visiblePages) {
    ...
    _pagingScrollView.contentOffset = [self contentOffseBBorPageAtIndex:indexPriorToLayout];
...
- (BBPhotoCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    BBPhotoCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <BBPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[BBPhotoCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (UIImageView *)imageView {
    BBZoomingScrollView *page = [self pageDisplayedAtIndex:_currentPageIndex];
    if (page) {
...
- (void)loadAdjacentPhotosIfNecessary:(id<BBPhoto>)photo {
    BBZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
...
//        }
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <BBPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    BBPLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1);
                }
            }
...
#pragma mark - BBPhoto Loading Notification

- (void)handleBBPhotoLoadingDidEndNotification:(NSNotification *)notification {
    NSLog(@"%s",__func__);
    id <BBPhoto> photo = [notification object];
    BBZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
...
- (void)handleSeletedTagInfoNotifiacation:(NSNotification*)notifiaction {
    if (_currentTagView) {
        BBPhotoTag *tag = notifiaction.object;
...
- (void)tilePages {
    ...
    for (BBZoomingScrollView *page in _visiblePages) {
        ...
        if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
            ...
            BBPLog(@"Removed page at index %lu", (unsigned long)pageIndex);
        }
    }
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            BBZoomingScrollView *page = [self dequeueRecycledPage];
            if (!page) {
                page = [[BBZoomingScrollView alloc] initWithPhotoBrowser:self];
            }
            ...
            BBPLog(@"Added page at index %lu", (unsigned long)index);
            
            // Add caption
            BBPhotoCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
            ...
            if (page.displayingVideo) {
                ...
                [playButton setImage:[UIImage imageForResourcePath:@"BBLibraryResource.bundle/images/PlayButtonOverlayLarge" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
                [playButton setImage:[UIImage imageForResourcePath:@"BBLibraryResource.bundle/images/PlayButtonOverlayLargeTap" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateHighlighted];
            ...
            if (self.displaySelectionButtons) {
                //BBLibraryCollectionUnSelected
                UIButton *selectedButton = [UIButton buttonWithType:UIButtonTypeCustom];
                [selectedButton setImage:BBPhotoBrowserImageNamed(@"BBLibraryCollectionUnSelected")
                ...
                if (self.customImageSelectedIconName) {
                    ...
                    selectedOnImage = BBPhotoBrowserImageNamed(@"BBLibraryCollectionSelected");
                }
...
- (void)updateVisiblePageStates {
    ...
    for (BBZoomingScrollView *page in copy) {
...
- (BBZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    BBZoomingScrollView *thePage = nil;
    for (BBZoomingScrollView *page in _visiblePages) {
        if (page.index == index) {
            thePage = page; break;
        }
...
- (BBZoomingScrollView *)dequeueRecycledPage {
    BBZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) {
            ...
            if (photo != [NSNull null]) {
                ...
                BBPLog(@"Released underlying image at index %lu", (unsigned long)i);
            }
        }
    }
    ...
    id <BBPhoto> currentPhoto = [self photoAtIndex:index];
...
- (CGRect)frameForCaptionView:(BBPhotoCaptionView *)captionView atIndex:(NSUInteger)index {
    ...
    CGSize captionSize = [captionView sizeThaBBits:CGSizeMake(pageFrame.size.width, 0)];
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        for (BBZoomingScrollView *page in _visiblePages) {
            if (page.captionView) {
                BBPhotoCaptionView *v = page.captionView;
...
- (void)addFaceRectOnView:(BBPhotoTag*)photoTag {
    ...
    BBPhotoTag *model = [BBPhotoTag photoTagWithRect:rect tagId:photoTag.tagId tagName:photoTag.tagName];
    ...
    if (array && [array isKindOfClass:[NSMutableArray class]]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                if ([obj isKindOfClass:[BBPhotoTag class]]) {
                    BBPhotoTag *model = (BBPhotoTag *)obj;
                    if (model) {
                        ...
                        dispatch_main_sync_safe(^{
                            ...
                            BBPhotoTagView *tagView = [[BBPhotoTagView alloc] initWithDelegate:self frame:CGRectMake(0, 0, model.tagRect.size.width / 2/scaleW,  model.tagRect.size.height  / 2/scaleW + 40)];
...
- (void)updateImageTagInfo:(BBPhotoTag *)model {
    NSMutableArray *array = [_tagInfos objectAtIndex:_currentPageIndex];
    if ([array isKindOfClass:[NSNull class]]) {
        //当前为空
        ...
        [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            if ([obj isKindOfClass:[BBPhotoTag class]]) {
                BBPhotoTag *oldModel = (BBPhotoTag *)obj;
...
#pragma mark BBPhotoTagViewDelegate
...
- (void)tagPopoverDidEndEditing:(BBPhotoTagView *)tagPopover {
    ...
    BBPhotoTag *model = [BBPhotoTag photoTagWithRect:rect tagId:tagPopover.tagId tagName:tagPopover.text];
    [self updateImageTagInfo:model];
}
